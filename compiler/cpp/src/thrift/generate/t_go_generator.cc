...
public:
  ...
  std::string function_signature_if(t_function* tfunction,
                                    std::string prefix = "",
                                    bool addError = false, bool is_client = false);
  ...
  std::string packaged_type_name(t_type* ttype, const std::string prefix, const std::string suffix);
...
std::string t_go_generator::packaged_type_name(t_type* ttype, const std::string prefix, const std::string suffix) {
    string tname = type_name(ttype);
    size_t index = tname.rfind(".");

    if (index != string::npos) {
      return tname.substr(0, index + 1) +
                  prefix + publicize(tname.substr(index + 1)) + suffix;
    }

    return prefix + publicize(tname) + suffix;
}
...
std::string t_go_generator::publicize(const std::string& value, bool is_args_or_result) const {
  ...
  if (!is_args_or_result) {
    ...
    bool ends_with_snk = (len_before >= 4) && (value2.substr(len_before - 4, 4) == "Sink");
    bool ends_with_strm = (len_before >= 6) && (value2.substr(len_before - 6, 6) == "Stream");
    if (ends_with_args || ends_with_rslt || ends_with_snk || ends_with_strm) {
      value2 += '_';
    }
  }
...
void t_go_generator::init_generator() {
  ...
  // Print header
  f_types_ << go_autogen_comment() << go_package() << render_includes(false) << endl;

  f_consts_ << go_autogen_comment() << go_package() << render_includes(true) << endl;

  f_consts_ << render_import_protection() << endl << endl;
...
void t_go_generator::generate_go_function_helpers(t_function* tfunction) {
  if (!tfunction->is_oneway()) {
    ...
    f_types_ << indent() << "func (p *" << tstruct_name << ") GetResult() interface{} {" << endl;
    indent_up();
    if (!tfunction->get_returntype()->is_void()) {
      f_types_ << indent() << "return p.GetSuccess()" << endl;
    } else {
      f_types_ << indent() << "return nil" << endl << endl;
    }
    indent_down();
    f_types_ << indent() << "}" << endl << endl;
    f_types_ << indent() << "func (p *" << tstruct_name << ") GetError() error {" << endl;
    indent_up();
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
      t_field* f = *f_iter;
      f->set_req(t_field::T_OPTIONAL);
      result.append(f);
      f_types_ << indent() << "if p." << publicize((*f_iter)->get_name()) << " != nil {" << endl;
      indent_up();
      f_types_ << indent() << "return p." << publicize((*f_iter)->get_name()) << endl;
      indent_down();
      f_types_ << indent() << "}" << endl << endl;
    }
    f_types_ << indent() << "return nil" << endl << endl;
  ...
  if (sink != NULL) {
    f_types_ << indent() << "type " << publicize(tfunction->get_name() + "SinkInboundStream", true) << " interface {" << endl;
    indent_up();
    f_types_ << indent() << "io.Closer" << endl << endl;
    f_types_ << indent() << "Receive(thrift.Context) ("
               << type_to_go_type(sink) << ", error)" << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    std::string istruct_name(privatize(tfunction->get_name() + "SinkInboundStream"));

    f_types_ << indent() << "type " << istruct_name << " struct {" << endl;
    indent_up();
    f_types_ << indent() << "thrift.TInboundStream" << endl << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    f_types_ << indent() << "func (p *" << istruct_name << ") Receive(ctx thrift.Context) (res "
               << type_to_go_type(sink) << ", err error) {" << endl;
    indent_up();
    std::string iresultname = publicize(tfunction->get_name() + "_sink", true);
    f_types_ << indent() << "result := " << iresultname << "{}" << endl;
    f_types_ << indent() << "if err := p.TInboundStream.Receive(ctx, &result); err != nil {"
                           << endl;
    indent_up();
    f_types_ << indent() << "return res, err"  << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;
    f_types_ << indent() << "return result.Arg, nil" << endl << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    f_types_ << indent() << "type " << publicize(tfunction->get_name() + "SinkOutboundStream", true) << " interface {" << endl;
    indent_up();
    f_types_ << indent() << "io.Closer" << endl << endl;
    f_types_ << indent() << "Send(thrift.Context, "
               << type_to_go_type(sink) << ") error" << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    std::string ostruct_name(privatize(tfunction->get_name() + "SinkOutboundStream"));

    f_types_ << indent() << "type " << ostruct_name << " struct {" << endl;
    indent_up();
    f_types_ << indent() << "thrift.TOutboundStream" << endl << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    f_types_ << indent() << "func (p *" << ostruct_name << ") Send(ctx thrift.Context, arg "
               << type_to_go_type(sink) << ") error {" << endl;
    indent_up();
    std::string oresultname = publicize(tfunction->get_name() + "_sink", true);
    f_types_ << indent() << "args := " << oresultname << "{Arg: arg}" << endl;
    f_types_ << indent() << "return p.TOutboundStream.Send(ctx, &args)" << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;
  ...
  if (stream_ != NULL) {
    f_types_ << indent() << "type " << publicize(tfunction->get_name() + "StreamInboundStream", true) << " interface {" << endl;
    indent_up();
    f_types_ << indent() << "io.Closer" << endl << endl;
    f_types_ << indent() << "Receive(thrift.Context) ("
               << type_to_go_type(stream_) << ", error)" << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;

    std::string istruct_name(privatize(tfunction->get_name() + "StreamInboundStream"));
    f_types_ << indent() << "type " << istruct_name << " struct {" << endl;
    indent_up();
    f_types_ << indent() << "thrift.TInboundStream" << endl << endl;
    indent_down();
    ...
    f_types_ << indent() << "func (p *" << istruct_name << ") Receive(ctx thrift.Context) (res "
               << type_to_go_type(stream_) << ", err error) {" << endl;
    indent_up();
    std::string iresultname = publicize(tfunction->get_name() + "_stream", true);
    f_types_ << indent() << "result := " << iresultname << "{}" << endl;
    f_types_ << indent() << "if err := p.TInboundStream.Receive(ctx, &result); err != nil {"
                           << endl;
    indent_up();
    ...
    f_types_ << indent() << "type " << publicize(tfunction->get_name() + "StreamOutboundStream", true) << " interface {" << endl;
    indent_up();
    f_types_ << indent() << "io.Closer" << endl << endl;
    f_types_ << indent() << "Send(thrift.Context, "
               << type_to_go_type(stream_) << ") error" << endl;
    ...
    std::string ostruct_name(privatize(tfunction->get_name() + "StreamOutboundStream"));
    f_types_ << indent() << "type " << ostruct_name << " struct {" << endl;
    indent_up();
    f_types_ << indent() << "thrift.TOutboundStream" << endl << endl;
    indent_down();
    ...
    f_types_ << indent() << "func (p *" << ostruct_name << ") Send(ctx thrift.Context, arg "
               << type_to_go_type(stream_) << ") error {" << endl;
    indent_up();
    std::string oresultname = publicize(tfunction->get_name() + "_stream", true);
    f_types_ << indent() << "args := " << oresultname << "{Arg: arg}" << endl;
    f_types_ << indent() << "return p.TOutboundStream.Send(ctx, &args)" << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;
...
void t_go_generator::generate_service_interface(t_service* tservice) {
  if (!tservice->is_streaming()) {
    string extends_if = "";
    string interfaceName(publicize(tservice->get_name()));

    if (tservice->get_extends() != NULL) {
      extends_if = "\n" + indent() +
                   packaged_type_name(tservice->get_extends(), "", "") + "\n";
    }
  ...
  std::string def_name(privatize(interfaceName + "ServiceDefinition"));

  f_types_ << indent() << "var " << def_name << " = thrift.ServiceDefinition{" << endl;
  indent_up();

  f_types_ << indent() << "Namespace: Namespace, " << endl;


  generate_go_annotated_definition(f_types_, tservice);

  if (!functions.empty()) {
    vector<t_function*>::iterator f_iter;
    f_types_ << indent() << "Functions: []thrift.FunctionDefinition{" << endl;

    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
      f_types_ << indent() << "{" << endl;
      indent_up();
      if ((*f_iter)->is_oneway()) {
        f_types_ << indent() << "IsOneway: true," << endl;
      } else {
        f_types_ << indent() << "Result: &" <<   privatize(publicize((*f_iter)->get_name() + "_result", true) + "StructDefinition") << "," << endl;
      }

      f_types_ << indent() << "Args: " <<   privatize(publicize((*f_iter)->get_name() + "_args", true) + "StructDefinition") << "," << endl;

      generate_go_annotated_definition(f_types_, *f_iter);
  ...
  f_const_values_ << "  thrift.RegisterService(" << def_name << ")" << endl;
    ...
    generate_go_docstring(f_types_, tservice);
    f_types_ << indent() << "type " << interfaceName << "Handler interface {";
    indent_up();
    f_types_ << interfaceName << endl;
    indent_down();
    f_types_ << indent() << "}" << endl << endl;
  } else {
    string extends_if_client = "";
    string extends_if_handler = "";
    string interfaceName(publicize(tservice->get_name()));

    if (tservice->get_extends() != NULL) {
      extends_if_client = "\n" + indent() +
                          packaged_type_name(tservice->get_extends(), "", "Client") + "\n";
      extends_if_handler = "\n" + indent() +
                           packaged_type_name(tservice->get_extends(), "", "Handler") + "\n";
    }
...
void t_go_generator::generate_service_client(t_service* tservice) {
  ...
  indent_up();
  f_types_ << indent() << "cl, err := p.Build(\"" << tservice->get_program()->get_namespace("*") << "\", \"" << tservice->get_name() << "\")" << endl;
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    case t_function::T_REQUEST_RESPONSE:
      ...
      for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
          ...
          if ((*f_iter)->get_returntype()->is_void()) {
            f_types_ << indent() << "return result." << pubname << endl;
          } else {
            f_types_ << indent() << "return res, result." << pubname << endl;
          }
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        ...
        f_types_ << indent() << "if !result.IsSetSuccess() {" << endl;
        indent_up();
    ...
    case t_function::T_STREAM_CLIENT:
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        ...
        f_types_ << indent() << "return res, nil, err" << endl;
      }
      f_types_ << indent() << "}" << endl << endl;
      ...
      for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
        ...
        if ((*f_iter)->get_returntype()->is_void()) {
          f_types_ << indent() << "  return nil, result." << pubname << endl;
        } else {
          f_types_ << indent() << "  return res, nil, result." << pubname << endl;
        }
        f_types_ << indent() << "}";
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        f_types_ << indent() << "return &"
                   << privatize((*f_iter)->get_name() + "SinkOutboundStream")
                   << "{TOutboundStream: clientSink}, nil" << endl;
      } else {
        f_types_ << indent() << "return result.GetSuccess(), &"
                   << privatize((*f_iter)->get_name() + "SinkOutboundStream")
                   << "{TOutboundStream: clientSink}, nil" << endl;
      }
      break;
    case t_function::T_STREAM_SERVER:
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        f_types_ << indent() << "return &"
                   << privatize((*f_iter)->get_name() + "StreamInboundStream")
                   << "{TInboundStream: clientStream}, nil" << endl;
      } else {
        f_types_ << indent() << "return result.GetSuccess(), &"
                   << privatize((*f_iter)->get_name() + "StreamInboundStream")
                   << "{TInboundStream: clientStream}, nil" << endl;
    ...
    case t_function::T_STREAM_BIDI:
      ...
      if ((*f_iter)->get_returntype()->is_void()) {
        ...
        f_types_ << indent() << "return res, nil, nil, err" << endl;
      }
      f_types_ << indent() << "}" << endl << endl;
      ...
      for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
        ...
        f_types_ << indent() << "if result." << pubname << " != nil {" << endl;
        if ((*f_iter)->get_returntype()->is_void()) {
          f_types_ << indent() << "  return nil, nil, result." << pubname << endl;
        } else {
          f_types_ << indent() << "  return res, nil, nil, result." << pubname << endl;
        }
        f_types_ << indent() << "}";
...
void t_go_generator::generate_service_server(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_types_ << indent() << "\"" << escapedFuncName << "\"," << endl;
    ...
    switch ((*f_iter)->get_rpc_type()) {
    case t_function::T_ONEWAY:
      f_types_ << indent() << "thrift.NewTUnaryProcessorFunction(p, \"" << escapedFuncName << "\", func() thrift.TRequest { return &" << argsname << "{}}, &" << processorName << "{handler: handler})," << endl;
      break;
    case t_function::T_REQUEST_RESPONSE:
      f_types_ << indent() << "thrift.NewTBinaryProcessorFunction(p, \"" << escapedFuncName << "\", func() thrift.TRequest { return &" << argsname << "{}}, &" << processorName << "{handler: handler})," << endl;
      break;
    case t_function::T_STREAM_CLIENT:
      f_types_ << indent() << "thrift.NewTStreamClientProcessorFunction(p, \"" << escapedFuncName << "\", func() thrift.TRequest { return &" << argsname << "{}}, &" << processorName << "{handler: handler})," << endl;
      break;
    case t_function::T_STREAM_SERVER:
      f_types_ << indent() << "thrift.NewTStreamServerProcessorFunction(p, \"" << escapedFuncName << "\", func() thrift.TRequest { return &" << argsname << "{}}, &" << processorName << "{handler: handler})," << endl;
      break;
    case t_function::T_STREAM_BIDI:
      f_types_ << indent() << "thrift.NewTStreamBidiProcessorFunction(p, \"" << escapedFuncName << "\", func() thrift.TRequest { return &" << argsname << "{}}, &" << processorName << "{handler: handler})," << endl;
      break;
    }
...
void t_go_generator::generate_process_function(t_service* tservice, t_function* tfunction) {
  ...
  case t_function::T_STREAM_CLIENT:
      f_types_ << indent() << "func (p *" << processorName
               << ") Handle(ctx thrift.Context, req thrift.TRequest, sink thrift.TInboundStream) (thrift.TResponse, error) {" << endl;
      break;
  case t_function::T_STREAM_SERVER:
      f_types_ << indent() << "func (p *" << processorName
               << ") Handle(ctx thrift.Context, req thrift.TRequest, stream thrift.TOutboundStream) (thrift.TResponse, error) {" << endl;
      break;
  case t_function::T_STREAM_BIDI:
      f_types_ << indent() << "func (p *" << processorName
               << ") Handle(ctx thrift.Context, req thrift.TRequest, sink  thrift.TInboundStream, stream thrift.TOutboundStream) (thrift.TResponse, error) {" << endl;
      break;
  }
  ...
  if (!tfunction->get_returntype()->is_void()) {
    ...
    f_types_ << indent() << "err2 := p.handler." << publicize(tfunction->get_name()) << "(ctx";
  }
  ...
  if (stream_ != NULL) {
     f_types_ << ", &"
                << privatize(tfunction->get_name() + "StreamOutboundStream") << "{TOutboundStream: stream}";
  }

  if (sink != NULL) {
     f_types_ << ", &"
                << privatize(tfunction->get_name() + "SinkInboundStream") << "{TInboundStream: sink}";
  }

  f_types_ << ")" << endl;

  if (tfunction->is_oneway()) {
    f_types_ << indent() << "return err2" << endl;
    f_types_ << indent() << "}" << endl;
  } else {
    f_types_ << indent() << "result := &" << resultname << "{}" << endl;
    ...
    if (!x_fields.empty()) {
      f_types_ << indent() << "switch v := thrift.Cause(err2).(type) {" << endl;
 ...
 */
string t_go_generator::function_signature_if(t_function* tfunction, string prefix, bool addError, bool is_client) {
  ...
  if (!is_client && stream_ != NULL) {
     signature += ", stream " + publicize(tfunction->get_name() + "StreamOutboundStream", true);
  }

  if (!is_client && sink != NULL) {
     signature += ", sink " + publicize(tfunction->get_name() + "SinkInboundStream", true);
  }
  ...
  if (is_client && stream_ != NULL) {
     signature += "stream " + publicize(tfunction->get_name() + "StreamInboundStream", true) + ", ";
  }

  if (is_client && sink != NULL) {
     signature += "sink " + publicize(tfunction->get_name() + "SinkOutboundStream", true) + ", ";
  }
