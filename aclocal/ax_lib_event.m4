         ...
         [
          ...
          if test "$want_libevent" = "yes"; then
            ...
            if test -n "$ax_libevent_path"; then
              ...
              for ax_libevent_path in "" $lt_sysroot/usr $lt_sysroot/usr/local $lt_sysroot/opt $lt_sysroot/opt/local $lt_sysroot/opt/libevent $lt_sysroot/opt/homebrew "$LIBEVENT_ROOT" ; do
                AX_LIB_EVENT_DO_CHECK
